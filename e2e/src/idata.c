    ...
    *hashmap_config = SYS_HASHMAP_CONFIG(SIZE_MAX, SYS_HASHMAP_DEFAULT_LOAD_FACTOR);
    ...
    for (size_t i = 0; i < interfaces_len; ++i) {
        ...
        idata_map_value_t initialized_value = (idata_map_value_t) {
            ...
            // the second parameter of SPSC_INITIALIZER needs to point to allocated_value
            .expected = SPSC_INITIALIZER(ARRAY_SIZE(initialized_value.expected_buf), (astarte_message_t * const) allocated_value->expected_buf),
            .expected_buf = { 0 },
        };
...
int idata_pop_individual(
    const char *interface_name, e2e_individual_data_t *out_individual)
{
    ...
    CHECK_RET_1(interface == NULL, "Unknown passed interface");
    CHECK_RET_1(interface->aggregation != ASTARTE_INTERFACE_AGGREGATION_INDIVIDUAL,
...
int idata_pop_property(
    const char *interface_name, e2e_property_data_t *out_property)
{
...
int idata_pop_object(const char *interface_name, e2e_object_data_t *out_object)
{
...
static void free_map_entry_callback(uint64_t key, uint64_t value, void *user_data) {
    ...
    if (data_value->interface->type == ASTARTE_INTERFACE_TYPE_PROPERTIES) {
        ...
        while(idata_pop_property(data_value->interface->name, &property) == 0) {
        ...
        while(idata_pop_object(data_value->interface->name, &object) == 0) {
            free_object(object);
        }
    }
    else if (data_value->interface->aggregation == ASTARTE_INTERFACE_AGGREGATION_INDIVIDUAL) {
        e2e_individual_data_t individual = {};
        while(idata_pop_individual(data_value->interface->name, &individual) == 0) {
