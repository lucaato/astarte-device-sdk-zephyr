...
void device_setup(astarte_device_config_t config)
{
    ...
    // we take the semaphore after initializing the device to make sure that errors from astarte_device_new
...
astarte_device_handle_t get_device() {
    CHECK_HALT(k_sem_count_get(&device_sem) > 0 || get_termination(), "The device is not initialized or is terminating");
...
static void device_thread_entry_point(void *unused1, void *unused2, void *unused3) {
    ...
    CHECK_ASTARTE_OK_HALT(astarte_device_connect(device_handle), "Astarte device connection failure.");
    ...
    CHECK_ASTARTE_OK_HALT(
        astarte_device_disconnect(device_handle, K_SECONDS(10)), "Astarte device disconnection failure.");
